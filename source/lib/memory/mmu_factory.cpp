...
MMU* MMUFactory::create(uint8_t* rom, uint32_t romSize, DMAMediator* dmaMediator, DMAMediator* hdmaMediator, InterruptMediator* interruptMediator, TimerHandler* timerHandler, bool isCGB)
{
    MemorySpace* cartridge = this->cartridgeFactory.create(rom, romSize);
    MemorySpace* highRam = this->createHighRam(isCGB);
    MemorySpace* ioRegisters = this->createIORegisters(dmaMediator, hdmaMediator, interruptMediator, timerHandler, isCGB);
...
MemorySpace* MMUFactory::createIORegisters(DMAMediator* dmaMediator, DMAMediator* hdmaMediator, InterruptMediator* interruptMediator, TimerHandler* timerHandler, bool isCGB)
{
    MemorySpace* ioRegisters = new IORegisters(dmaMediator, hdmaMediator, interruptMediator, timerHandler, isCGB);
